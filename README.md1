...
<head>
  ...
  <style>
    * { margin:0; padding:0; box-sizing:border-box; -webkit-tap-highlight-color:transparent; user-select:none; }
    body { font-family:-apple-system,'Segoe UI',Roboto,Helvetica,Arial,sans-serif; background:#0a0e1a; color:#d0d9e6; overflow:hidden; height:100vh; width:100vw; position:fixed; }
    ...
    .screen { position:absolute; top:0; left:0; width:100%; height:100%; padding:20px 16px 80px 16px; overflow-y:auto; opacity:0; pointer-events:none; transition:opacity 0.3s ease, transform 0.3s ease; transform:scale(0.97); background:transparent; will-change:transform,opacity; }
    .screen.active { opacity:1; pointer-events:auto; transform:scale(1); }
    .screen-header { display:flex; align-items:center; gap:12px; padding:8px 0 16px 0; border-bottom:1px solid rgba(40,70,120,0.25); margin-bottom:16px; flex-shrink:0; }
    .screen-header .back-btn { background:none; border:none; color:#6a8aaa; font-size:24px; cursor:pointer; padding:4px 8px; transition:color 0.2s; line-height:1; }
    .screen-header .back-btn:hover { color:#aac8ee; }
    ...
    /* PIN */
    #pinScreen { display:flex; flex-direction:column; justify-content:center; align-items:center; padding:40px 20px; background:radial-gradient(ellipse at center, #0f1a2e, #050810); opacity:1 !important; pointer-events:auto !important; transform:none !important; }
    ...
    .pin-dot { width:16px; height:16px; border-radius:50%; border:2px solid #3a5a7a; background:transparent; transition:background 0.15s ease, border-color 0.15s ease; }
    ...
    .pin-key { width:70px; height:70px; border-radius:50%; border:1px solid rgba(60,100,150,0.3); background:rgba(20,40,70,0.5); color:#b0cce6; font-size:26px; font-weight:300; cursor:pointer; transition:background 0.15s, transform 0.1s; display:flex; align-items:center; justify-content:center; backdrop-filter:blur(4px); }
    .pin-key:active { background:rgba(60,120,200,0.3); transform:scale(0.92); }
    .pin-key.empty { background:transparent !important; border:none !important; pointer-events:none; }
    ...
    .mandala-item { aspect-ratio:1; background:rgba(20,45,80,0.4); border-radius:20px; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:4px; border:1px solid rgba(60,120,200,0.15); cursor:pointer; transition:background 0.2s, transform 0.15s, border-color 0.2s; backdrop-filter:blur(2px); padding:8px; text-align:center; }
    .mandala-item:active { transform:scale(0.94); background:rgba(40,80,140,0.3); border-color:rgba(80,160,240,0.3); }
    ...
    .mandala-center { grid-column:2; grid-row:2; aspect-ratio:1; background:radial-gradient(circle, #1a3a5a, #0f1a2e); border-radius:50%; display:flex; align-items:center; justify-content:center; border:2px solid rgba(80,160,240,0.3); cursor:pointer; transition:transform 0.2s, border-color 0.2s, box-shadow 0.2s; box-shadow:0 0 30px rgba(40,100,200,0.1); font-size:36px; color:#8ab4d6; }
    .mandala-center:active { transform:scale(0.92); border-color:rgba(80,200,255,0.6); box-shadow:0 0 50px rgba(60,150,255,0.2); }

    /* FORMS */
    .form-group { margin-bottom:16px; }
    .form-group label { display:block; font-size:12px; color:#6a8aaa; margin-bottom:4px; letter-spacing:0.5px; font-weight:500; }
    .form-group input, .form-group textarea { width:100%; padding:12px 14px; background:rgba(15,30,55,0.7); border:1px solid rgba(50,90,140,0.2); border-radius:12px; color:#d0d9e6; font-size:15px; outline:none; transition:border-color 0.2s; font-family:inherit; }
    .form-group input:focus, .form-group textarea:focus { border-color:rgba(80,160,240,0.4); }
    .form-group textarea { resize:vertical; min-height:70px; }
    .form-group input::placeholder, .form-group textarea::placeholder { color:#4a5a7a; }

    .checkbox-grid { display:flex; flex-wrap:wrap; gap:8px; margin-top:4px; }
    .checkbox-item { display:flex; align-items:center; gap:6px; background:rgba(20,45,80,0.3); padding:6px 12px; border-radius:20px; border:1px solid rgba(50,90,140,0.15); font-size:13px; color:#8aabca; cursor:pointer; transition:background 0.2s, border-color 0.2s; }
    .checkbox-item.active { background:rgba(40,100,180,0.25); border-color:rgba(80,160,240,0.4); color:#b0d0ee; }
    .checkbox-item input { display:none; }
    .checkbox-item .check { width:16px; height:16px; border-radius:4px; border:2px solid #4a6a8a; display:flex; align-items:center; justify-content:center; font-size:10px; transition:background 0.2s, border-color 0.2s; flex-shrink:0; }
    ...
    .photo-upload-btn { display:inline-flex; align-items:center; gap:8px; padding:8px 16px; background:rgba(30,60,110,0.3); border:1px dashed rgba(60,120,200,0.3); border-radius:12px; color:#6a8aaa; cursor:pointer; font-size:13px; transition:background 0.2s; }
    .photo-upload-btn:active { background:rgba(40,80,160,0.2); }

    .btn-primary { width:100%; padding:14px; background:linear-gradient(135deg, #1a4a7a, #0f2a4a); border:1px solid rgba(60,140,220,0.2); border-radius:14px; color:#c8dcee; font-size:16px; font-weight:500; cursor:pointer; transition:background 0.2s, transform 0.1s; letter-spacing:0.5px; margin-top:8px; }
    .btn-primary:active { transform:scale(0.97); background:linear-gradient(135deg, #1a5a8a, #0f2a5a); }
    ...
    .finance-tree .folder { padding:6px 8px; cursor:pointer; border-radius:8px; transition:background 0.15s; font-size:14px; display:flex; align-items:center; gap:8px; }
    ...
    .finance-add input { padding:8px 10px; background:rgba(15,30,55,0.7); border:1px solid rgba(50,90,140,0.2); border-radius:10px; color:#d0d9e6; font-size:13px; outline:none; width:100%; }
    .finance-add input:focus { border-color:rgba(80,160,240,0.4); }
    .finance-add .add-btn { grid-column:1 / -1; padding:10px; background:rgba(30,80,150,0.3); border:1px solid rgba(60,120,200,0.2); border-radius:10px; color:#8ab4d6; font-size:14px; cursor:pointer; transition:background 0.2s; text-align:center; }
    .finance-add .add-btn:active { background:rgba(40,100,180,0.3); }
    ...
    .festival-card .status { font-size:11px; padding:4px 10px; border-radius:20px; background:rgba(40,80,130,0.2); color:#8aabca; white-space:nowrap; cursor:pointer; transition:background 0.2s; }
    .festival-card .status:active { background:rgba(60,120,200,0.2); }
    .festival-card .delete-fest { background:none; border:none; color:#5a4a4a; font-size:18px; cursor:pointer; padding:0 4px; }

    /* CALENDAR */
    .cal-grid { display:grid; grid-template-columns:repeat(7, 1fr); gap:4px; margin-bottom:12px; }
    .cal-grid .cal-header { text-align:center; font-size:10px; color:#4a6a8a; padding:4px 0; font-weight:600; letter-spacing:0.5px; }
    .cal-grid .cal-day { aspect-ratio:1; display:flex; align-items:center; justify-content:center; border-radius:50%; font-size:14px; color:#8aabca; cursor:pointer; transition:background 0.15s; background:transparent; border:none; font-family:inherit; position:relative; }
    ...
    .cal-event-item .event-check { width:20px; height:20px; border-radius:6px; border:2px solid #3a5a7a; display:flex; align-items:center; justify-content:center; cursor:pointer; font-size:12px; transition:background 0.2s, border-color 0.2s; flex-shrink:0; }
    ...
    .notes-grid .note-card .note-label { font-size:11px; color:#4a6a8a; margin-bottom:6px; font-weight:600; letter-spacing:0.5px; }
    ...
    .social-item .check-btn { padding:6px 16px; background:rgba(30,80,150,0.3); border:1px solid rgba(60,120,200,0.2); border-radius:20px; color:#8ab4d6; font-size:12px; cursor:pointer; transition:background 0.2s; }
    .social-item .check-btn:active { background:rgba(40,100,180,0.3); }

    /* GALLERY */
    .gallery-grid { display:grid; grid-template-columns:repeat(3, 1fr); gap:6px; }
    .gallery-grid .gallery-item { aspect-ratio:1; border-radius:10px; overflow:hidden; background:rgba(20,45,80,0.3); border:1px solid rgba(40,80,130,0.1); display:flex; align-items:center; justify-content:center; font-size:24px; color:#3a5a7a; position:relative; }
    .gallery-grid .gallery-item img { width:100%; height:100%; object-fit:cover; }
    .gallery-grid .gallery-item .del-gallery { position:absolute; top:2px; right:2px; background:rgba(0,0,0,0.5); border:none; color:#c87070; border-radius:50%; width:20px; height:20px; font-size:12px; cursor:pointer; display:flex; align-items:center; justify-content:center; }
    ...
    .settings-item input:focus { border-color:rgba(80,160,240,0.4); }
    .settings-item .save-btn { padding:6px 16px; background:rgba(30,80,150,0.3); border:1px solid rgba(60,120,200,0.2); border-radius:8px; color:#8ab4d6; font-size:12px; cursor:pointer; transition:background 0.2s; }
    .settings-item .save-btn:active { background:rgba(40,100,180,0.3); }
...
<div class="app-container" id="appContainer">
  ...
  <div class="screen" id="mandalaScreen">
    ...
    <div class="mandala-grid">
      <div class="mandala-item" data-screen="calendarScreen"><div class="icon">📅</div><div class="label">Календарь</div></div>
      <div class="mandala-item" data-screen="festivalScreen"><div class="icon">🎪</div><div class="label">Фестивали</div></div>
      <div class="mandala-item" data-screen="notesScreen"><div class="icon">📝</div><div class="label">Заметки</div></div>
      <div class="mandala-item" data-screen="settingsScreen"><div class="icon">⚙️</div><div class="label">Настройки</div></div>
      <div class="mandala-center" id="centerPlus">＋</div>
      <div class="mandala-item" data-screen="financeScreen"><div class="icon">💰</div><div class="label">Финансы</div></div>
      <div class="mandala-item" data-screen="socialScreen"><div class="icon">🌐</div><div class="label">Мои соцсети</div></div>
      <div class="mandala-item" data-screen="galleryScreen"><div class="icon">🖼️</div><div class="label">Галерея</div></div>
    </div>
  </div>
...
<script>
  ...
  function saveData() {
    try { localStorage.setItem('elfData', JSON.stringify(DATA)); updateDataSize(); } catch(e) {}
  }
  function loadData() {
    try {
  ...
  document.getElementById('centerPlus').addEventListener('click', () => showScreen('publishScreen'));
  ...
  function handlePinInput(val) {
    if (val === 'del') { pinBuffer = pinBuffer.slice(0,-1); updatePinDots(); pinError.classList.remove('show'); return; }
  ...
  const GREETINGS = ['Рисуй чаще, и чаща нарисует тебе','Не бойся, я с тобой','С возвращением, мастер Эльфа!','Творчество начинается здесь','Вдохновение уже рядом'];
  function showGreeting() {
    ...
    setTimeout(() => { overlay.classList.remove('show'); document.getElementById('pinScreen').classList.add('hidden'); showScreen('mandalaScreen'); }, 2200);
  }
  ...
  document.querySelectorAll('.checkbox-item').forEach(el => el.addEventListener('click', () => el.classList.toggle('active')));
  ...
  document.getElementById('publishBtn').addEventListener('click', async () => {
    ...
    const platforms = [...document.querySelectorAll('.checkbox-item.active')].map(e => e.dataset.platform);
    ...
    for (const p of platforms) {
      ...
      if (plat && p !== 'vk') { window.open(plat.url, '_blank'); await new Promise(r => setTimeout(r, 300)); }
    }
    ...
    function renderNode(node, path) {
      ...
      keys.forEach(key => {
        ...
        if (isSel) {
          ...
          if (subNode) renderNode.call({ tree: subNode }, subNode, [...path, key]);
  ...
  document.getElementById('calEventAdd').addEventListener('click', () => {
    ...
    if (!text) return;
    if (!calSelectedDate) calSelectedDate = `${new Date().getFullYear()}-${String(new Date().getMonth()+1).padStart(2,'0')}-${String(new Date().getDate()).padStart(2,'0')}`;
  ...
  document.querySelectorAll('.notes-grid textarea').forEach(ta => {
    ...
    ta.addEventListener('input', () => { DATA.notes[ta.dataset.note] = ta.value; saveData(); });
  });
  ...
  function renderGallery() {
    ...
    grid.innerHTML = DATA.gallery.map((src,i) => `<div class="gallery-item"><img src="${src}" alt="Работа"><button class="del-gallery" data-idx="${i}">✕</button></div>`).join('');
  ...
  // ==================== INIT ====================
  loadData(); updateDataSize();
  document.getElementById('pinScreen').classList.add('active');
  const today2 = new Date(); calSelectedDate = `${today2.getFullYear()}-${String(today2.getMonth()+1).padStart(2,'0')}-${String(today2.getDate()).padStart(2,'0')}`;
  ...
  console.log('🦉 Мастерская Эльфа v2.0 — чек-листы живы!');
</script>
</body>
</html>
