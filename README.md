...
<head>
  ...
  <!-- Манифест PWA (иконка с кисточкой) -->
...
<div class="app" id="app">
  ...
  <div id="mainScreen" class="screen">
    ...
    <input type="text" id="postTags" placeholder="Хештеги (через запятую)">
...
<script>
  (function() {
    ...
    let state = {
      ...
      currentStep: 0,
      platforms: [
        { id: 'vk', name: 'ВКонтакте', url: 'https://vk.com/wall' },
        ...
        { id: 'artallery', name: 'Артальберио', url: 'https://artallery.ru/add' },
        { id: 'meshok', name: 'Мешок', url: 'https://meshok.net/new_item' },
        { id: 'pikabu', name: 'Пикабу', url: 'https://pikabu.ru/add' }
      ]
    };
    ...
    function loadState() {
      ...
      if (!state.pin) state.pin = '1234'; // пин по умолчанию
    }
    function saveState() {
    ...
    document.getElementById('loginBtn').addEventListener('click', () => {
      ...
      if (entered === state.pin) {
        ...
        alert('Неверный пин, попробуй ещё раз');
      }
    });
    ...
    document.getElementById('resetPinBtn').addEventListener('click', () => {
      ...
      if (newPin && /^\d{4}$/.test(newPin)) {
        state.pin = newPin;
        saveState();
        alert('Пин обновлён! Теперь используй его для входа.');
      } else {
        alert('Пин должен быть ровно из 4 цифр.');
      }
    });
    ...
    function showGreeting() {
      ...
      document.getElementById('greetingText').textContent = randomPhrase;
    ...
    function showNextPlatform() {
      ...
      progressDiv.innerHTML = `
        <div class="step">
          ...
          <span>${state.currentStep + 1}/${state.selectedPlatforms.length}</span>
        </div>
        <p style="color:#f0dbb0; margin:8px 0;">Открываем площадку...</p>
      `;

      // Копируем в буфер текст и первое фото (если есть)
      copyToClipboard(platform).then(() => {
        // Открываем площадку в новой вкладке
        window.open(platform.url, '_blank');
        // Ждём возврата (пользователь нажмёт кнопку "Далее")
        progressDiv.innerHTML += `
          <p style="color:#b8a68b;">Вставь текст и фото (Ctrl+V) и опубликуй, затем нажми:</p>
          <button class="btn" id="nextStepBtn" style="margin-top:8px;">✅ Готово, следующая</button>
        `;
    ...
    async function copyToClipboard(platform) {
      ...
      try {
        ...
        // Если есть фото, пытаемся скопировать первое как изображение (может не работать на некоторых устройствах)
        ...
        console.warn('Копирование в буфер не удалось, но текст уже скопирован.', err);
      }
    }
    ...
    // Рендер платформ при активации mainScreen
